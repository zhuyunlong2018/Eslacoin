	...
	<view class="uni-navbar">
		<view :class="{ 'uni-navbar--fixed': fixed, 'uni-navbar--shadow': shadow, 'uni-navbar--border': border }" :style="{ 'background': backgroundColor }"
		 class="uni-navbar__content">
			...
			<view :style="{ color: color,background: backgroundColor }" class="uni-navbar__header uni-navbar__content_view">
				...
				<view class="uni-navbar__header-container uni-navbar__content_view" @tap="onClickTitle">
					<view class="uni-navbar__header-container-inner uni-navbar__content_view" v-if="title.length">
						<text class="uni-nav-bar-text no-wrap" :style="{color: color }">{{ title }}</text>
					</view>
					<!-- 标题插槽 -->
					<slot />
				</view>
...
<script>
	...
	export default {
		...
		props: {
			...
			backgroundColor: {
				type: String,
				default: "linear-gradient(90deg, #2571ea 0%, #2c53eb 100%)"
			},
			statusBar: {
				type: [Boolean, String],
				default: true
