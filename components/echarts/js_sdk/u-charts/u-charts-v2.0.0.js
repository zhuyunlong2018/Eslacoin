...
function getToolTipData(seriesData, opts, index, categories) {
  ...
  var textList = seriesData.map(function(item) {
	...
	// 判断是否存在符号
	if (opts.symbol) {
		item.data = item.data + opts.symbol;
	}
    return {
    ...
    for (let i = 0; i < splitPointList[0].length; i++) {
      if (i > leftNum && i < rightNum) {
        ...
        if (data[i][1] - data[i][0] > 0) {
          ...
          context.lineTo(item[0].x - eachSpacing / 4, item[0].y); //开盘左侧点 + 10
          context.lineTo(item[0].x, item[0].y); //开盘中间点 + 10
          context.lineTo(item[2].x, item[2].y); //底点 + 10
          context.lineTo(item[0].x, item[0].y); //开盘中间点 + 10
          context.lineTo(item[0].x + eachSpacing / 4, item[0].y); //开盘右侧点 + 10
          ...
          context.lineTo(item[1].x - eachSpacing / 4, item[1].y); //收盘左侧点 + 20
          context.lineTo(item[1].x, item[1].y); //收盘中间点 + 20
          context.lineTo(item[2].x, item[2].y); //底点s + 20
          context.lineTo(item[1].x, item[1].y); //收盘中间点 + 20
          context.lineTo(item[1].x + eachSpacing / 4, item[1].y); //收盘右侧点 + 20
          context.lineTo(item[0].x + eachSpacing / 4, item[0].y); //开盘右侧点
          context.lineTo(item[0].x, item[0].y); //开盘中间点
          context.moveTo(item[3].x, item[3].y); //顶点
		  // let t = `顶点：${item[3].x + ',' +item[3].y}; 
				// 	开盘中间点：${item[0].x + ',' +item[0].y}
				// 	开盘左侧点：${item[0].x - eachSpacing / 4 + ',' +item[0].y}；
				// 	收盘左侧点：${item[1].x - eachSpacing / 4 + ',' +item[1].y}；
				// 	收盘中间点：${item[1].x + ',' +item[1].y};
				// 	 底点：${item[2].x + ',' +item[2].y}; 
				// 	 收盘中间点：${item[1].x + ',' +item[1].y}; 
				// 	 收盘右侧点：${item[1].x + eachSpacing / 4 + ',' +item[1].y}；
				// 	 开盘右侧点：${item[0].x + eachSpacing / 4 + ',' +item[0].y}；
				// 	开盘中间点：${item[0].x + ',' +item[0].y};
				// 	顶点：${item[3].x + ',' +item[3].y}; 
				// 	 `
