	...
	<view class="root flex">
		...
		<view class="item2 flex">
			<view class="left ">
				...
				<text class="font-15 color-light p-lr10">现货-DCA交易</text>
			</view>
			<view class="flex ">
		...
		<view class="item2 flex">
			<view class="left ">
				...
				<text class="font-15 color-light p-lr10">{{TaskData.TriggerNum==-1?'无限次':TaskData.TriggerNum+'次'}}</text>
			</view>
			<view class="" >
				<text class="font-13 color-dark" v-if="TaskData.TriggerCondition==2">价格区间:</text>
	...
	export default {
		...
		computed: {
			...
			replishText() {
				...
				let count = ReplishData.map(item => item.BuyCoin).reduce((a, b) => {
					return a + b
				});
				return '<text class="font-15 color-light p-l10"> ' + len +
					...
					'</text><text class="font-10 color-dark"> USDT</text>'
			},
