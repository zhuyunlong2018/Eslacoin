<template>
	<view class="root">
		...
		<view class="edit-view" :class="{'edit-view-active':FocusInput=='editBuyCount'}">
			<view class="edit-view-left">
				<text class="font-13 color-dark">每次购买量：</text>
			</view>
			<input class="edit-view-center text-left color-light font-15" type="number"
				@focus="(e)=>focus1(e,'editBuyCount')" @blur="(e)=>blur1(e,'editBuyCount')"
				v-model="TaskData.BuyCount"></input>
			<view class="edit-view-right flex flex-end">
				<text class="font-13 color-light"></text>
				<text class="wen iconfont iconwenhao"
					@click="showTextClick('每次开单购买总数量。')"></text>
			</view>
		</view>
		...
		<view class="edit-view" :class="{'edit-view-active':FocusInput=='editSellRatio'}">
			...
			<view class="edit-view-right flex flex-end">
				...
				<text class="wen iconfont iconwenhao"
					@click="showTextClick('每单达到该盈利率后止盈，该值受杠杆倍数影响较大。(例如杠杆20倍，现货价格上涨1%，则合约收益率为20%)')"></text>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			onBgfxClick() {
				...
				this.GdFxParam.BuyDirection = this.TaskData.BuyDirection
				this.GdFxParam.Count = parseFloat(this.TaskData.BuyCount)
				this.$refs.gdfx.open()
			},
			//打开自定义picker
			openPicker(type) {
